...
function LeaveType() {
  ...
  const handleEdit = (leavetype) => {
    ...
    setEditId(leavetype._id);
    setValues({ leavetype: leavetype.leavetype });
  };
      ...
      try {
        ...
        if (editId) {
          response = await axios.patch(
            ...
            { leavetype: values.leavetype, leavetype_id: editId },
            {
              headers: {
                Authorization: `Token ${token}`,
                "Content-Type": "application/json",
          ...
          response = await axios.post(baseURL + API_URLS.leavetypeapi, values, {
            headers: {
              Authorization: `Token ${token}`,
              "Content-Type": "application/json",
            },
  ...
  const handleDelete = async (leavetype_id) => {
    ...
    try {
      ...
      const response = await axios.delete(baseURL + API_URLS.leavetypeapi, {
        headers: {
          Authorization: `Token ${token}`,
      ...
      <div className="">
        <div className="w-[98%] mx-auto bg-white shadow-md rounded-lg ">
          ...
          <form
            ...
            className=" mb-6 text-sm text-gray-700 px-6 "
          >
            <div className="grid grid-cols-2">
              <div className="flex flex-col mb-4">
                <label className="font-medium text-sm">Leave Type Name</label>
                <input
                  type="text"
                  placeholder=""
