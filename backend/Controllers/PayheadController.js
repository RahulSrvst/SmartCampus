...
const add_paytype = async (req,res) =>{
    try{
        ...
        const newPayhead = new PayheadModel({payheadtype,description,addition_or_deduction});
...
const get_paytype = async(req,res)=>{
    try{
        const {id} = req.query;
        if(id){
            const paytype = await PayheadModel.findById(id);
        ...
        return res.status(200).json({
            ...
            const paytype = await PayheadModel.find();
