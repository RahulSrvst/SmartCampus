...
const get_attendance = async (req, res) => {
    ...
    if (!collegeId) {
      ...
      if (id) {
        const attendance_data = await EmployeeAttendanceModel.findOne({_id:id,collegeId})
      ...
      if (employee_id) {
        const data = await EmployeeAttendanceModel.find({ employee: employee_id ,collegeId})
          .populate("employee", "firstname lastname")
          .populate("designation", "designation_name");
      ...
      if (designation_id) {
        const data = await EmployeeAttendanceModel.find({ designation: designation_id , collegeId})
          .populate("employee", "firstname lastname")
          .populate("designation", "designation_name");
      ...
      const attendance_data = await EmployeeAttendanceModel.find({collegeId})
        .populate("employee", "firstname lastname")
        .populate("designation", "designation_name");
  ...
  const get_employee_attendance = async (req, res) => {
    ...
    if (!collegeId) {
      ...
      // Query for payslips of all employees or a specific employee
      const payslipQuery = employee_id
      ...
      // If payslip data exists, add it to the employee's data
      payslipData.forEach((payslip) => {
        const employeeId = payslip.employee.toString();
        if (employeeGroup[employeeId]) {
          employeeGroup[employeeId].payslip = payslip;
        }
      });
